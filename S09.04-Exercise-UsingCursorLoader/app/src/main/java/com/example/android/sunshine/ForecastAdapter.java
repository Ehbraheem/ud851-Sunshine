...
//      TODO (6) Move the cursor to the appropriate position
        mCorsor.moveToPosition(position);
//      TODO (7) Generate a weather summary with the date, description, high and low
        long dateInmills = mCorsor.getLong(MainActivity.INDEX_WEATHER_DATE);
        String dateString = SunshineDateUtils.getFriendlyDateString(mContext, dateInmills, false);

        int weatherId = mCorsor.getInt(MainActivity.INDEX_WEATHER_CONDITION_ID);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext, weatherId);

        double highInCelcius = mCorsor.getDouble(MainActivity.INDEX_WEATHER_MAX_TEMP);

        double lowInCelsius = mCorsor.getDouble(MainActivity.INDEX_WEATHER_MIN_TEMP);

        String highAndLowTemperature =
                SunshineWeatherUtils.formatHighLows(mContext, highInCelcius, lowInCelsius);
